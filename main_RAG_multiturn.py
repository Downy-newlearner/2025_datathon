...
def main(num):
    # API 키 로드
    load_dotenv()
    api_key = os.getenv('UPSTAGE_API_KEYS')
    if not api_key:
    ...
    # 데이터 로드
    test = pd.read_csv(os.path.join(base_config.data_dir, f'test.csv')) # test.csv에서 100개의 데이터를 샘플링한 데이터입니다.
    ...
    result_df = pd.read_csv(f"1st_result_{template_name}.csv")
    results = []
    ...
    output_df.to_csv(f"submission_few_shot6_ver2.csv", index=False)
